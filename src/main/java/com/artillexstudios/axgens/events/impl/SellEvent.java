...
public class SellEvent extends Event {
    ...
    public SellEvent(EventConfig config) {
        super(config);

        multiplier = ((Number) config.options().get("multiplier")).doubleValue();
    }
    ...
    public void onStart() {
        ...
        String multiplier = String.valueOf((Math.round(100 * Double.parseDouble(getConfig().options().get("multiplier").toString()))));
    ...
    public void onCancel() {
        ...
        Map<String, String> replacements = Collections.singletonMap("%multiplier%", String.valueOf(multiplier));
    ...
    public void tick() {
        String multiplier = String.valueOf((Math.round(100 * Double.parseDouble(getConfig().options().get("multiplier").toString()))));
        bossBar.setTitle(StringUtils.formatToString(getConfig().bossBar().replace("%multiplier%", multiplier).replace("%bonus-tiers%", String.valueOf(multiplier)).replace("%time%", "" + ((endTime - System.currentTimeMillis()) / 1_000L))));
